...
var url = 'http://sms-nhsy.ehijoy.com/sms/send?game=nhsy&type=nhsy_monitor&phone='    
  ...
  'order': {
    ...
    'func3' : function (orders, cb) {
      if (common.sms) {
        ...
        var content = '(霓虹)' + keys + '在' + getTime() + '下单失败'
  ...
  'thirdCallback': {
    ...
    'func3' : function (tcs, cb) {
      if (common.sms) {
        ...
        var content = '霓虹' + keys.toString() + '在' + getTime() + '回调失败'
        sendSms(common.smsList, content)
      }
      cb('ok', tcs)
    }
